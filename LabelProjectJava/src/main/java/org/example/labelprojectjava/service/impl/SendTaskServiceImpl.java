...
public class SendTaskServiceImpl implements SendTaskService {
    ...
    @Override
    public void sendTask(String fileUrl) {
        getInfo(Thread.currentThread().getName());

        getInfo("Processing file: " + fileUrl);

        String userUUID = getUserWithLuaScript(fileUrl);
    ...
    private String getUserWithLuaScript(String fileUrl) {
        String luaScript =
        ...
        List<String> keys = Arrays.asList("user:status", "uploadFile:userDo:" + fileUrl);
